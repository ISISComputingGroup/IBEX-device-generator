...
def grant_permissions_for_github_repository(device: DeviceInfo, github_token: str) -> None:
    ...
    grant_permission(github_token, "ICP-Write", "push", device.substitutions[GITHUB_REPO_NAME])
    grant_permission(github_token, "ICP-WriteAndMerge", "maintain", device.substitutions[GITHUB_REPO_NAME])
...
def does_github_issue_exist_and_is_open(issue_number: int) -> bool:
    ...
    """
    result =  requests.get(f"https://api.github.com/repos/{ORGANIZATION_NAME}/IBEX/issues/{issue_number}")
